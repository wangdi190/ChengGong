...
//将一个多边形转化为一个四个点的矩形
void FileV30::AllPtTo4Pt(DPOINT *in,int ptsum,DPOINT *out)
{
	CDRect drt;
	drt=CreateNRt(ptsum,in);
	if(drt.Width()<3)
	{
		drt.left-=5;
		drt.right+=5;
	...
	out[0].x=drt.left; out[0].y=drt.top;
	out[1].x=drt.right;out[1].y=drt.top;
	out[2].x=drt.right;out[2].y=drt.bottom;
	out[3].x=drt.left; out[3].y=drt.bottom;
}



//返回版本3保存所需字节数
int FileV30::GetV3Size(DObj &bs,int mode)
{
	int m,n;
	N_DBX *dbx;
	N_TEXT *txt;
	...
	case 1: //多边形
		dbx=(N_DBX*)bs.buf;
		n=dbx->ptsum;
		m=m+sizeof(N3_DBX)+n*sizeof(DPOINT);
		break;
	case 2: case 3://矩形，园
		m=m+sizeof(N3_RECT);
		break;
	...
	case 16://路径
		path=(N_PATH*)bs.buf;
		n=path->ptsum;
		m=m+sizeof(N3_PATH)+n*sizeof(DPOINT);
		break;
	case 17://静态小图标
		m=m+sizeof(N3_SPIC);
		break;
	case 32:
		m=m+sizeof(N3_METER1);
		break;
	case 33:
		m=m+sizeof(N3_METER2);
		break;
	case 34:
		m=m+sizeof(N3_METER3);
		break;
	case 35:
		m=m+sizeof(N3_METER4);
		break;
	case 64://模拟量显示结构
		m=m+sizeof(N3_ANL);
		break;
	...
	case 71:case 72://日期格式,时间格式
		...
		break;
	case 73://动画图形
		m=m+sizeof(N3_GIFAML);
		break;
	case 74://滚动文本
		stxt=(N_SRLTEXT*)bs.buf;
		n=strlen(stxt->text);
		m=m+sizeof(N3_SRLTEXT)+n;
		break;
	case 75://按钮格式
		m=m+sizeof(N3_PUSH);
		break;
	case 76://自助控件
		m=m+sizeof(N3_SELFCTL);
		break;
	case 77://潮流线
		flw=(N_FLOW*)bs.buf;
		n=flw->ptsum;
		m=m+sizeof(N3_FLOW)+n*sizeof(DPOINT);
		break;
	case 78://连接线
		con=(N_CONLINE*)bs.buf;
		n=con->ptsum;
		m=m+sizeof(N3_CONLINE)+n*sizeof(DPOINT);
		break;
	case 79://母线
		m=m+sizeof(N3_BUS);
		break;
	case 80://小型活动对象
		m=m+sizeof(N3_ACTOBJ);
		break;
	case 81://FLASH对象
		m=m+sizeof(N3_FLASH);
		break;
	case 82://百分比饼图
		m=m+sizeof(N3_PCTPIE);
		break;
	}
	return m;
}
...
int FileV30::V2DbxToBs(void *lp, void *lp2)
{
	...
	AllPtTo4Pt(d1->pt,d1->ptsum,d2->pta);
	return s;
}
...
int FileV30::V2PathToBs(void *lp, void *lp2)
{
	...
	AllPtTo4Pt(p1->pt,p1->ptsum,p2->pta);
...
int FileV30::V2ElegToBs(void *lp, void *lp2)
{
	...
	e2->did=e1->did;
	e2->egrpid=dbm->GetEGPRecNo(e1->egrpid);
	e2->genrev=e1->genrev;
...
int FileV30::V2ConlToBs(void *lp, void *lp2)
{
	...
	AllPtTo4Pt(c1->pt,c1->ptsum,c2->pta);
...
int FileV30::V3TextToBs(void *lp, void *lp2)
{
	...
	if(t1->zm!=1&&t1->zm!=0){
		t2->font.lfHeight=(int)(t1->font.lfHeight);
		t2->font.lfWidth=(int)(t1->font.lfWidth);
	}
	t2->zm=1;
	strcpy(t2->text,t1->text);
...
int FileV30::V3AnlToBs(void *lp, void *lp2)
{
	...
	a2->lf=a1->lf;
	if(a1->zm!=1&&a1->zm!=0){
		a2->lf.lfWidth=(int)(a2->lf.lfWidth/a1->zm);
		a2->lf.lfHeight=(int)(a2->lf.lfHeight/a1->zm);
	}
	a2->ncolor=a1->ncolor;
	a2->xs=a1->xs;
	a2->xsmode=a1->xsmode;
...
int FileV30::V3ReservoirToBs(void *lp, void *lp2)
{
	...
	AllPtTo4Pt(a1->pt,a1->ptsum,a2->pta);
	memset(&a2->vr,0,sizeof(READDEV));
	return size;
}
...
int FileV30::V3YBToBs(void *lp, void *lp2)
{
	...
	for(i=0;i<8;i++){
		...
		e2->umr[i].ptype=e1->umr[i].ptype;
		e2->umr[i].vmin=e1->umr[i].vmin;
		e2->umr[i].vmax=e1->umr[i].vmax;
		e2->umr[i].alarm=e1->umr[i].alarm;
		for(j=0;j<4;j++) e2->umr[i].val[j]=e1->umr[i].val[j];
		for(j=0;j<2;j++) e2->umr[i].color[j]=e1->umr[i].color[j];
	}
	e2->show=1;
	GetptFromDRECT(e1->rt,e2->pt);
...
//////////////////////////////////////////////////////////////////////
void FileV30::GetptFromDRECT(DRECT&rt,DPOINT *pt)
{
...
int FileV30::V4FlowToBs(void *lp, void *lp2)
{
	...
	f2->type=f1->type;
	f2->vl=f1->vl;
	f2->linewidth=0;//f1->linewidth>4 ? 0:f1->linewidth;
	memcpy(f2->pt,f1->pt,f1->ptsum*sizeof(DPOINT));
	AllPtTo4Pt(f1->pt,f1->ptsum,f2->pta);
	return sizeof(N_FLOW)+f1->ptsum*sizeof(DPOINT);
}
...
int FileV30::V4AnlToBs(void *lp, void *lp2)
{
	...
	a2->vr.did=a1->did;
	a2->vr.dtype=a1->dtype;
	a2->vr.xsmode=(unsigned)a1->xsmode;
	a2->lf=a1->lf;
	if(a1->zm!=1&&a1->zm!=0){
		a2->lf.lfWidth=(int)(a2->lf.lfWidth/a1->zm);
		a2->lf.lfHeight=(int)(a2->lf.lfHeight/a1->zm);
...
int FileV30::V5YBToBs(void *lp, void *lp2)
{
	...
	for(i=0;i<8;i++){
		...
		e2->umr[i].ptype=e1->pam[i].ptype;
		e2->umr[i].vmin=e1->pam[i].vmin;
		e2->umr[i].vmax=e1->pam[i].vmax;
		e2->umr[i].alarm=e1->pam[i].alarm;
		for(j=0;j<4;j++) e2->umr[i].val[j]=e1->pam[i].val[j];
		for(j=0;j<2;j++) e2->umr[i].color[j]=e1->pam[i].color[j];
	}
	e2->show=1;
	GetptFromDRECT(e1->rt,e2->pt);
	return sizeof(N_ELEGRPYB);
